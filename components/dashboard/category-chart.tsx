...
import { categories, currency } from "@/lib/finance-data"
    ...
    <div className="rounded-3xl bg-card p-5 text-card-foreground">
      ...
      <div className="relative mx-auto mt-4 w-full">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-xl font-semibold tracking-tight">{currency(total)}</span>
        </div>
      </div>
      ...
      <ul className="mt-4 space-y-2.5">
        {categories.map((c) => (
          <li key={c.name} className="flex items-center gap-2 text-sm">
            ...
            <span className="ml-auto font-medium">{currency(c.value)}</span>
